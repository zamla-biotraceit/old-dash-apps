...
def update_overlay_graph(p1_overlay_sheets, p3_overlay_sheets, p4_overlay_sheets, n_clicks, 
    ...
    try:
        ...
        if any([p1_overlay_sheets, p3_overlay_sheets, p4_overlay_sheets]):
            neg_deltas = {'P1': [], 'P3': [], 'P4': []}
            pos_deltas = {'P1': [], 'P3': [], 'P4': []}
            
            for phase, sheets in [('P1', p1_overlay_sheets), ('P3', p3_overlay_sheets), ('P4', p4_overlay_sheets)]:
                for sheet in sheets:
                    if sheet in sheet_data:
                        df = sheet_data[sheet]['data']
                        if not df.empty:
                            peaks = detect_peaks_rolling_window(df, sheet)
                            if not peaks.empty:
                                neg_deltas[phase].extend(peaks['neg_delta'].dropna().tolist())
                                pos_deltas[phase].extend(peaks['pos_delta'].dropna().tolist())
            
            # Create negative deltas graph
            neg_data = []
            for phase in ['P1', 'P3', 'P4']:
                if neg_deltas[phase]:
                    neg_data.append(go.Bar(
                        name=phase,
                        x=[phase],
                        y=[abs(np.mean(neg_deltas[phase]))],
                        error_y=dict(
                            type='data',
                            array=[np.std(neg_deltas[phase])],
                            visible=True
                        )
                    ))
            ...
            for phase in ['P1', 'P3', 'P4']:
                if pos_deltas[phase]:
                    pos_data.append(go.Bar(
                        name=phase,
                        x=[phase],
                        y=[np.mean(pos_deltas[phase])],
                        error_y=dict(
                            type='data',
                            array=[np.std(pos_deltas[phase])],
                            visible=True
                        )
                    ))
            ...
            # Event set mappings for color coding
            event_sets = {
            'Event Set 1: Walk': {
                'P1': '1 Event',
                'P3': '3 Event', 
            ...
            # Create individual negative deltas graph with event-based coloring
            indiv_neg_data = {}  # Use dict to group traces by event set
            for phase in ['P1', 'P3', 'P4']:
                for sheet in [s for s in sheet_data.keys() if phase in s]:
                    sheet_data_df = sheet_data[sheet]['data'].copy()
                    if not sheet_data_df.empty:
                        # Apply filters
                        filtered_data = sheet_data_df.copy()
                        ...
                        if not filtered_data.empty:
                            peaks_df = detect_peaks_rolling_window(filtered_data, sheet)
                            if not peaks_df.empty:
                                for set_name, set_info in event_sets.items():
                                    phase_event = set_info.get(phase)
                                    if phase_event:
                                        event_data = peaks_df[peaks_df['Event'] == phase_event]
                                        event_deltas = event_data['neg_delta'].dropna()
                                        
                                        if not event_deltas.empty:
                                            if set_name not in indiv_neg_data:
                                                indiv_neg_data[set_name] = {
                                                    'x': [], 'y': [], 'text': [], 
                                                    'color': set_info['color']
                                                }
                                            
                                            x_vals = [f"{phase}_{i+1}" for i in range(len(event_deltas))]
                                            y_vals = [abs(val) for val in event_deltas]
                                            text_vals = [f'Δ: {abs(val):.1f}<br>Offset: {offset:.1f}' 
                                                        for val, offset in zip(event_deltas, event_data['Offset'])]
                                            
                                            indiv_neg_data[set_name]['x'].extend(x_vals)
                                            indiv_neg_data[set_name]['y'].extend(y_vals)
                                            indiv_neg_data[set_name]['text'].extend(text_vals)

            # Create traces with combined data for each event set
            indiv_neg_traces = []
            for set_name, data in indiv_neg_data.items():
            ...
            indiv_neg_fig.update_layout(
                title='Individual Negative Peak Deltas by Phase and Event',
                yaxis_title='Absolute Delta Value',
                xaxis_title='Sample',
                showlegend=True,
                barmode='group',
            ...
            # Create individual positive deltas graph with event-based coloring
            indiv_pos_data = {}  
            for phase in ['P1', 'P3', 'P4']:
                for sheet in [s for s in sheet_data.keys() if phase in s]:
                    sheet_data_df = sheet_data[sheet]['data'].copy()
                        ...
                        if not filtered_data.empty:
                            ...
                            if not peaks_df.empty:
                                for set_name, set_info in event_sets.items():
                                    ...
                                    if phase_event:
                                        event_data = peaks_df[peaks_df['Event'] == phase_event]
                                        event_deltas = event_data['pos_delta'].dropna()
                                        
                                        if not event_deltas.empty:
                                            if set_name not in indiv_pos_data:
                                                indiv_pos_data[set_name] = {
                                                    'x': [], 'y': [], 'text': [], 
                                                    'color': set_info['color']
                                                }
                                            ...
                                            x_vals = [f"{phase}_{i+1}" for i in range(len(event_deltas))]
                                            y_vals = event_deltas
                                            text_vals = [f'Δ: {val:.1f}<br>Offset: {offset:.1f}' 
                                                        for val, offset in zip(event_deltas, event_data['Offset'])]
                                            
                                            indiv_pos_data[set_name]['x'].extend(x_vals)
                                            indiv_pos_data[set_name]['y'].extend(y_vals)
                                            indiv_pos_data[set_name]['text'].extend(text_vals)

            indiv_pos_traces = []
            for set_name, data in indiv_pos_data.items():
            ...
            indiv_pos_fig.update_layout(
                title='Individual Positive Peak Deltas by Phase and Event',
                yaxis_title='Delta Value', 
                xaxis_title='Sample',
                showlegend=True,
                barmode='group',
        ...
        return main_fig, toggle_text, toggle_style, indiv_neg_fig, indiv_pos_fig, neg_fig, pos_fig
